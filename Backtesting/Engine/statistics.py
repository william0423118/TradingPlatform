...
class Stat:
    ...
    def compute(self, time_range = 'all', isPlot = False):
        '''
        ...
        total_result = self._compute_one_period(self._data.index,
                                        date_size/250,
                                        rf = np.exp(self._market_info['int_rate']/250)-1)
        ...
        agg = df.groupby([per])
        for month, groups in agg:
            tmp = agg.get_group(month)
            # print(tmp)
    ...
    def _sharpe_ratio(self, ret, rf=0.005):
        """
    ...
    def _sterling_ratio(self, data, ret, rf=0.005):
