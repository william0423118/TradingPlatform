...
import pandas as pd
import numpy as np
from Backtesting.Engine.statistics import Stat

def alpha():
    pass

class Market:
    def __init__(self, start_date, end_date, cash, isClose = True, commision = 0.0015, rf = 0.03, universe=None):
    ...
    def adjustPosition(self, new_position):
        ...
        commision = np.abs((self.today_price.iloc[:, 0] * positionChange).dropna()*self.commision).sum()
...
if __name__=='__main__':
    mkt = Market(start_date='2017-10-16',
                 end_date='2018-11-15',
                 cash=1000000)
    print(mkt.cash_account, mkt.portfolio_account, mkt.market_account)
    position = pd.DataFrame([-1,0],index=['AAPL','GOOG'])
    ...
    while True:
        ...
        print(mkt.get_date())
        print(mkt.cash_account, mkt.portfolio_account, mkt.market_account)
        if mkt.get_date() == '2018-10-30':
            position = pd.DataFrame([-10, 100], index=['AAPL', 'GOOG'])
            mkt.adjustPosition(position)
        if ret == 0:
            break

    a, b = mkt.stats.compute('monthly')
    print(b)